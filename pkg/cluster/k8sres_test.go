...
func TestGenerateSpiloJSONConfiguration(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			role:     "zalandos",
			opConfig: config.Config{},
			result:   `{"postgresql":{"bin_dir":"/usr/lib/postgresql/9.6/bin"},"bootstrap":{"initdb":[{"auth-host":"md5"},{"auth-local":"trust"}],"users":{"zalandos":{"password":"","options":["CREATEDB","NOLOGIN"]}},"dcs":{"synchronous_commit":"","synchronous_standby_names":"","synchronous_mode":"","synchronous_mode_strict":""}}}`,
		},
		{
			...
			patroni: &acidv1.Patroni{
				...
				SynchronousMode:         "on",
				SynchronousModeStrict:   "on",
				Slots:                   map[string]map[string]string{"permanent_logical_1": {"type": "logical", "database": "foo", "plugin": "pgoutput"}},
			},
			role:     "zalandos",
			opConfig: config.Config{},
			result:   `{"postgresql":{"bin_dir":"/usr/lib/postgresql/11/bin","pg_hba":["hostssl all all 0.0.0.0/0 md5","host    all all 0.0.0.0/0 md5"]},"bootstrap":{"initdb":[{"auth-host":"md5"},{"auth-local":"trust"},"data-checksums",{"encoding":"UTF8"},{"locale":"en_US.UTF-8"}],"users":{"zalandos":{"password":"","options":["CREATEDB","NOLOGIN"]}},"dcs":{"ttl":30,"loop_wait":10,"retry_timeout":10,"maximum_lag_on_failover":33554432,"slots":{"permanent_logical_1":{"database":"foo","plugin":"pgoutput","type":"logical"}},"synchronous_commit":"on","synchronous_standby_names":"*","synchronous_mode":"on","synchronous_mode_strict":"on"}}}`,
		},
	}
